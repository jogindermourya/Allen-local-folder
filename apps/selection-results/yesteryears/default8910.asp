...
<%
	...
	rsRemarks.ActiveConnection = MM_Conn_STRING8910
	
	set Conn = Server.CreateObject("ADODB.Connection")
	Conn.Open MM_Conn_STRING8910
	
	
	If Len(Trim(strC)) = 0 Then
		'rsRemarks.Source = "SELECT * FROM REMARKS8910 WHERE EXAM = " & "'" & strE & "'"  & " AND SELYEAR = " & CDbl(strY)
		
		rsRemarks.open "{Call usp_getdata(5,'" & strE & "','" & strY & "','')}", Conn, adOpenStatic	
		
	Else
		'rsRemarks.Source = "SELECT * FROM REMARKS8910 WHERE EXAM = " & "'" & strE & "'" & " AND CATEGORY = " & "'" & strC & "'"  & " AND SELYEAR = " & CDbl(strY)
		
		rsRemarks.open "{Call usp_getdata(6,'" & strE & "','" & strY & "','" & strC & "')}", Conn, adOpenStatic	
	End If
	...
	rsResults.ActiveConnection = MM_Conn_STRING8910
	If Len(Trim(strC)) = 0 Then
		'rsResults.Source = "SELECT * FROM RESULTS8910 WHERE EXAM = " & "'" & strE & "'"  & " AND SELYEAR = " & CDbl(strY) & " ORDER BY SRNO"
		
		rsResults.open "{Call usp_getdata(7,'" & strE & "','" & strY & "','')}", Conn, adOpenStatic	
	Else
		'rsResults.Source = "SELECT * FROM RESULTS8910 WHERE EXAM = " & "'" & strE & "'" & " AND CATEGORY = " & "'" & strC & "'"  & " AND SELYEAR = " & CDbl(strY) & " ORDER BY SRNO"
		rsResults.open "{Call usp_getdata(8,'" & strE & "','" & strY & "','" & strC & "')}", Conn, adOpenStatic	
	End If
